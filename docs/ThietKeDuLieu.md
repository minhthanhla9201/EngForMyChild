...
> **Icon KHÔNG phụ thuộc font hệ thống**, render bằng `<img>`. Thứ tự ưu tiên (`.icon_src`, hàm `_resolve_icon_src` dùng chung): **(1)** ảnh upload (`media`, phụ huynh tự đặt) → **(2)** SVG tĩnh `icon_static` trong `web/static/icons/` (MẶC ĐỊNH — **commit theo repo** nên deploy máy khác luôn có, không cần tải mạng) → **(3)** SVG offline của emoji trong `media/images/` (nếu đã `fetch_ui_icons`) → **(4)** `''` (template hiện ký tự emoji). Vì sao dùng static cho mặc định: `media/` bị `.gitignore` (dữ liệu người dùng) nên không theo repo; SVG cố định phải nằm ở `static/` mới đi cùng mã nguồn. Tiện ích: `core/icons.py`, template tag `{% icon_img %}` (`core/templatetags/icons.py`).
